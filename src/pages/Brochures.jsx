...
const Brochures = () => {
  ...
  const fetchBrochures = async (params = {}) => {
    ...
    try {
      ...
      if (response.success) {
        setBrochures(response.data.brochures || response.data || []);
  ...
  const handleDelete = async () => {
    try {
      const response = await brochureService.deleteBrochure(selectedBrochure._id);
      if (response.success) {
      ...
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm md:text-base">
              Manage and organize your company brochures and downloadable materials
            </p>
          </div>
          <PermissionWrapper resource="brochures" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
              onClick={() => navigate('/brochures/new')}
        ...
        <Card className="border border-gray-200 shadow-md bg-white">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="flex-1 w-full">
              <Search
                placeholder="Search brochures by title or description..."
                allowClear
        ...
        >
          <Table
            ...
            loading={loading}
            rowKey="_id"
            className="custom-table brochures-table"
            pagination={{
              ...pagination,
              showSizeChanger: true,
              showQuickJumper: true,
              showTotal: (total, range) => 
                `${range[0]}-${range[1]} of ${total} brochures`,
            ...
            onRow={(record) => ({
              ...
              className: hasPermission('brochures', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
        ...
        <ConfirmModal
          ...
          content={`Are you sure you want to delete "${selectedBrochure?.title}"? This action cannot be undone.`}
