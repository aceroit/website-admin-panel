...
const CertificationRowActions = ({ record, onNavigate, onDeleteClick }) => {
  ...
  const menuItems = [];
  if (hasPermission('certifications', 'update') && workflowStatus.canEdit.canEdit) {
    menuItems.push({ key: 'edit', label: 'Edit', icon: <EditOutlined />, onClick: () => onNavigate(`/certifications/${record._id}`) });
  }
  if (hasPermission('certifications', 'delete') && workflowStatus.canDelete.canDelete) {
...
const Certifications = () => {
  ...
  const fetchCertifications = async (params = {}) => {
    ...
    try {
      ...
      if (response.success) {
        setCertifications(response.data.certifications || response.data || []);
  ...
  const handleDelete = async () => {
    try {
      const response = await certificationService.deleteCertification(selectedCertification._id);
      if (response.success) {
  ...
  const handleSearch = (value) => {
    ...
    fetchCertifications({ search: value });
  };
    ...
    },
    {
      ...
      render: (_, record) => (
        <CertificationRowActions
          record={record}
          onNavigate={navigate}
          onDeleteClick={(r) => { setSelectedCertification(r); setIsDeleteModalOpen(true); }}
        />
      ),
    },
  ];
      ...
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm md:text-base">
              Manage and organize your certifications and awards
            </p>
          </div>
          <PermissionWrapper resource="certifications" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
        ...
        >
          <Table
            ...
            loading={loading}
            rowKey="_id"
            className="custom-table certifications-table"
            pagination={{
              ...pagination,
              showSizeChanger: true,
              showQuickJumper: true,
              showTotal: (total, range) => 
                `${range[0]}-${range[1]} of ${total} certifications`,
            ...
            onRow={(record) => ({
              onClick: () => {
                if (hasPermission('certifications', 'update')) {
                  navigate(`/certifications/${record._id}`);
                }
              },
              className: hasPermission('certifications', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
        ...
        <ConfirmModal
          ...
          content={`Are you sure you want to delete "${selectedCertification?.name}"? This action cannot be undone.`}
