...
const CompanyUpdateCategoryRowActions = ({ record, onNavigate, onDeleteClick }) => {
  ...
  const menuItems = [];
  if (hasPermission('company-update-categories', 'update') && workflowStatus.canEdit.canEdit) {
    menuItems.push({ key: 'edit', label: 'Edit', icon: <EditOutlined />, onClick: () => onNavigate(`/company-update-categories/${record._id}`) });
  }
  if (hasPermission('company-update-categories', 'delete') && workflowStatus.canDelete.canDelete) {
...
const CompanyUpdateCategories = () => {
  ...
  const handleDelete = async () => {
    try {
      const response = await companyUpdateCategoryService.deleteCompanyUpdateCategory(selectedCategory._id);
      if (response.success) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 120,
      render: (_, record) => (
        <CompanyUpdateCategoryRowActions
          record={record}
          onNavigate={navigate}
          onDeleteClick={(r) => { setSelectedCategory(r); setIsDeleteModalOpen(true); }}
        />
      ),
    },
  ];
      ...
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-4xl font-bold text-gray-900 mb-2">
              Company Update Categories
            </h1>
            <p className="text-gray-500 text-sm md:text-base">
              Manage categories for organizing company updates and news
            </p>
          </div>
          <PermissionWrapper resource="company-update-categories" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
              onClick={() => navigate('/company-update-categories/new')}
        ...
        >
          <Table
            ...
            loading={loading}
            rowKey="_id"
            className="custom-table company-update-categories-table"
            pagination={{
              ...pagination,
            ...
            onRow={(record) => ({
              onClick: () => {
                if (hasPermission('company-update-categories', 'update')) {
                  navigate(`/company-update-categories/${record._id}`);
                }
              },
              className: hasPermission('company-update-categories', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
...
export default CompanyUpdateCategories;
