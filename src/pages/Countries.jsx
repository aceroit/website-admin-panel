...
const CountryRowActions = ({ record, onNavigate, onDeleteClick }) => {
  ...
  const menuItems = [];
  if (hasPermission('countries', 'update') && workflowStatus.canEdit.canEdit) {
    menuItems.push({ key: 'edit', label: 'Edit', icon: <EditOutlined />, onClick: () => onNavigate(`/countries/${record._id}`) });
  }
  if (hasPermission('countries', 'delete') && workflowStatus.canDelete.canDelete) {
...
const Countries = () => {
  ...
  const columns = [
      ...
      render: (_, record) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <span className="text-xs text-gray-500 truncate">Code: {record.code}</span>
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      key: 'isVisible',
      render: (isVisible) => (
        <Tag color={isVisible ? 'green' : 'red'} icon={isVisible ? <EyeOutlined /> : <EyeInvisibleOutlined />}>
          {isVisible ? 'Yes' : 'No'}
        </Tag>
      ),
      width: 100,
    ...
    },
    {
      ...
      render: (_, record) => (
        <CountryRowActions
          ...
          onDeleteClick={(r) => { setSelectedCountry(r); setIsDeleteModalOpen(true); }}
        />
      ),
    },
  ];
      ...
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm md:text-base">
              Manage countries for location references
            </p>
          </div>
          <PermissionWrapper resource="countries" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
        ...
        >
          <Table
            ...
            loading={loading}
            rowKey="_id"
            className="custom-table countries-table"
            ...
            onRow={(record) => ({
              onClick: () => {
                if (hasPermission('countries', 'update')) {
                  navigate(`/countries/${record._id}`);
                }
              },
              className: hasPermission('countries', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
        ...
        <ConfirmModal
          ...
          content={`Are you sure you want to delete "${selectedCountry?.name}"? This action cannot be undone.`}
