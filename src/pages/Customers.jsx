...
const Customers = () => {
  ...
  const handleDelete = async () => {
    try {
      const response = await customerService.deleteCustomer(selectedCustomer._id);
      if (response.success) {
    ...
    <MainLayout>
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <PermissionWrapper resource="customers" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
        ...
        >
          <Table
            ...
            onRow={(record) => ({
              ...
              className: hasPermission('customers', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
        ...
        <ConfirmModal
          ...
          content={`Are you sure you want to delete "${selectedCustomer?.name}"? This action cannot be undone.`}
