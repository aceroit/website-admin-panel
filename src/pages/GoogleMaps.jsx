...
const GoogleMapsRowActions = ({ record, onNavigate, onDeleteClick }) => {
  ...
  const menuItems = [];
  if (hasPermission('google-maps', 'update') && workflowStatus.canEdit.canEdit) {
    menuItems.push({ key: 'edit', label: 'Edit', icon: <EditOutlined />, onClick: () => onNavigate(`/website-configurations/maps/${record._id}`) });
  }
  if (hasPermission('google-maps', 'delete') && workflowStatus.canDelete.canDelete) {
...
const GoogleMaps = () => {
  ...
  const fetchConfigurations = async (params = {}) => {
    setLoading(true);
    try {
      const response = await googleMapsService.getAllGoogleMaps({
      ...
      toast.error(error.response?.data?.message || 'Failed to fetch Google Maps configurations');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      const response = await googleMapsService.deleteGoogleMaps(selectedConfig._id);
      if (response.success) {
        toast.success('Google Maps configuration deleted successfully');
        setIsDeleteModalOpen(false);
        setSelectedConfig(null);
        fetchConfigurations();
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to delete Google Maps configuration');
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-900">{record.title || 'Google Maps'}</span>
              {record.featured && (
            ...
            <span className="text-xs text-gray-500">
              {record.enabled?.value ? 'Enabled' : 'Disabled'} • API Key: {record.apiKey?.value ? 'Configured' : 'Not Set'}
            </span>
          </div>
        </div>
      ),
  ...
  return (
    <MainLayout>
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm md:text-base">
              Manage Google Maps API key configurations
            </p>
          </div>
          <PermissionWrapper resource="google-maps" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
              onClick={() => navigate('/website-configurations/maps/new')}
        ...
        <Card className="border border-gray-200 shadow-md bg-white" bodyStyle={{ padding: 0 }}>
          <Table
            columns={columns}
            dataSource={configurations}
            loading={loading}
            rowKey="_id"
            className="custom-table google-maps-table"
            pagination={{
              ...pagination,
            ...
            onRow={(record) => ({
              onClick: () => {
                if (hasPermission('google-maps', 'update')) {
                  navigate(`/website-configurations/maps/${record._id}`);
                }
              },
              className: hasPermission('google-maps', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
        ...
        <ConfirmModal
          ...
          title="Delete Google Maps Configuration"
