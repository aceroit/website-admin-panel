...
const GoogleReCaptchas = () => {
  ...
  const fetchConfigurations = async (params = {}) => {
    setLoading(true);
    try {
      const response = await googleReCaptchaService.getAllGoogleReCaptchas({
      ...
      toast.error(error.response?.data?.message || 'Failed to fetch Google ReCaptcha configurations');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      const response = await googleReCaptchaService.deleteGoogleReCaptcha(selectedConfig._id);
      if (response.success) {
        toast.success('Google ReCaptcha configuration deleted successfully');
        setIsDeleteModalOpen(false);
        setSelectedConfig(null);
        fetchConfigurations();
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to delete Google ReCaptcha configuration');
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-900">{record.title || 'Google ReCaptcha'}</span>
              {record.featured && (
            ...
            <span className="text-xs text-gray-500">
              Version: {record.version?.value || 'N/A'} • {record.enabled?.value ? 'Enabled' : 'Disabled'}
            </span>
          </div>
        </div>
      ),
      ...
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm md:text-base">
              Manage Google ReCaptcha configurations for bot protection
            </p>
          </div>
          <PermissionWrapper resource="google-recaptcha" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
              onClick={() => navigate('/website-configurations/recaptcha/new')}
        ...
        <Card className="border border-gray-200 shadow-md bg-white" bodyStyle={{ padding: 0 }}>
          <Table
            ...
            loading={loading}
            rowKey="_id"
            className="custom-table google-recaptcha-table"
            pagination={{
              ...pagination,
            ...
            onRow={(record) => ({
              ...
              className: hasPermission('google-recaptcha', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
        ...
        <ConfirmModal
          ...
          title="Delete Google ReCaptcha Configuration"
...
export default GoogleReCaptchas;
