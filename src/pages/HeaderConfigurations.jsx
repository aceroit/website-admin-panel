...
const HeaderConfigurations = () => {
  ...
  // Fetch header configurations
  const fetchConfigurations = async (params = {}) => {
    setLoading(true);
    try {
      const response = await headerConfigurationService.getAllHeaderConfigurations({
      ...
      toast.error(error.response?.data?.message || 'Failed to fetch header configurations');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      const response = await headerConfigurationService.deleteHeaderConfiguration(selectedConfig._id);
      if (response.success) {
        toast.success('Header configuration deleted successfully');
        setIsDeleteModalOpen(false);
        setSelectedConfig(null);
        fetchConfigurations();
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to delete header configuration');
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-900">{record.title || 'Header Configuration'}</span>
              {record.featured && (
            ...
            <span className="text-xs text-gray-500">
              Brand: {record.brandName?.text || 'N/A'}
            </span>
          </div>
        </div>
      ),
      ...
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm md:text-base">
              Manage website header configurations
            </p>
          </div>
          <PermissionWrapper resource="header-configurations" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
              onClick={() => navigate('/website-configurations/header/new')}
        ...
        >
          <Table
            ...
            loading={loading}
            rowKey="_id"
            className="custom-table header-configurations-table"
            pagination={{
              ...pagination,
            ...
            onRow={(record) => ({
              ...
              className: hasPermission('header-configurations', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
        ...
        <ConfirmModal
          ...
          title="Delete Header Configuration"
