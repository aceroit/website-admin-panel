...
const RegionRowActions = ({ record, onNavigate, onDeleteClick }) => {
  ...
  const menuItems = [];
  if (hasPermission('regions', 'update') && workflowStatus.canEdit.canEdit) {
    menuItems.push({ key: 'edit', label: 'Edit', icon: <EditOutlined />, onClick: () => onNavigate(`/regions/${record._id}`) });
  }
  if (hasPermission('regions', 'delete') && workflowStatus.canDelete.canDelete) {
...
const Regions = () => {
  ...
  const fetchRegions = async (params = {}) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Failed to fetch regions');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      const response = await regionService.deleteRegion(selectedRegion._id);
      if (response.success) {
    ...
    },
    {
      ...
      render: (_, record) => (
        <RegionRowActions
          ...
          onDeleteClick={(r) => { setSelectedRegion(r); setIsDeleteModalOpen(true); }}
        />
      ),
    },
  ];
      ...
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm md:text-base">
              Manage regions for location references
            </p>
          </div>
          <PermissionWrapper resource="regions" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
        ...
        >
          <Table
            ...
            loading={loading}
            rowKey="_id"
            className="custom-table regions-table"
            ...
            onRow={(record) => ({
              onClick: () => {
                if (hasPermission('regions', 'update')) {
                  navigate(`/regions/${record._id}`);
                }
              },
              className: hasPermission('regions', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
        ...
        <ConfirmModal
          ...
          title="Delete Region"
          content={`Are you sure you want to delete "${selectedRegion?.name}"? This action cannot be undone.`}
