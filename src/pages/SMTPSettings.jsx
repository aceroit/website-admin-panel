...
const SMTPSettings = () => {
  ...
  const fetchSettings = async (params = {}) => {
    setLoading(true);
    try {
      const response = await smtpSettingsService.getAllSMTPSettings({
      ...
      if (response.success) {
        const list = Array.isArray(response.data?.smtpSettings)
  ...
  const handleDelete = async () => {
    try {
      const response = await smtpSettingsService.deleteSMTPSettings(selectedSetting._id);
      if (response.success) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="font-medium text-gray-900">{record.title || 'SMTP Settings'}</span>
              {record.featured && (
      ...
      <div className="space-y-6 md:space-y-8 p-4 md:p-0">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm md:text-base">
              Manage SMTP configurations for outbound email
            </p>
          </div>
          <PermissionWrapper resource="smtp-settings" action="create">
            <Button
              type="primary"
              icon={<PlusOutlined />}
              size="large"
              onClick={() => navigate('/website-configurations/smtp/new')}
        ...
        <Card className="border border-gray-200 shadow-md bg-white">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="flex-1 w-full">
              <Search
                placeholder="Search settings by title or host..."
                allowClear
        ...
        <Card className="border border-gray-200 shadow-md bg-white" bodyStyle={{ padding: 0 }}>
          <Table
            ...
            loading={loading}
            rowKey="_id"
            className="custom-table smtp-settings-table"
            pagination={{
              ...pagination,
            ...
            onRow={(record) => ({
              ...
              className: hasPermission('smtp-settings', 'update') ? 'cursor-pointer hover:bg-gray-50' : '',
            })}
          />
        </Card>
        ...
        <ConfirmModal
          ...
          content={`Are you sure you want to delete "${selectedSetting?.title || 'this configuration'}"? This action cannot be undone.`}
